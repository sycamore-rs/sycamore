...
To create a signal, we use `ctx.create_signal(...)`. Note that the return value of this method is
...
```rust
let state = ctx.create_signal(0); // Create a reactive atom with an initial value of `0`.
...
ctx.create_effect(|| println!("The state changed. New value: {}", state.get()));
