...
// /// Run the async future inside a hydration context. If already inside a hydration context,
...
//     /// Every time this is incremented, `current_id` is reset to `0`. This is to add more
// tolerance     /// to hydration so that one component that doesn't hydrate correctly will not
// prevent other     /// components from hydrating.
//     pub current_component_id: usize,
//     /// The next component id. We need to save this because exiting the component scope
// decrements     /// the current component id. This is to ensure that component ids are unique for
