...
#[wasm_bindgen_test]
fn empty_template() {
    sycamore::render_to(&test_container(), |cx| View::empty(cx));
    assert_eq!(query("#test-container").inner_html(), "<!---->");
}

#[wasm_bindgen_test]
fn hello_world() {
    sycamore::render_to(&test_container(), |cx| {
        view! { cx,
            p { "Hello World!" }
        }
    });
...
fn template_interpolation_if_else() {
    create_scope_immediate(|cx| {
        ...
        sycamore::render_to(&test_container(), |_| node);
        assert_text_content!(query("p"), "Hello Sycamore!");
...
fn reactive_property() {
    create_scope_immediate(|cx| {
        ...
        let node = view! { cx,
            input(_type="checkbox", prop:indeterminate=*indeterminate.get())
        };
...
fn two_way_bind_to_value_as_number() {
    create_scope_immediate(|cx| {
        ...
        let node = view! { cx,
            input(_type="range", bind:valueAsNumber=value) // Note that type must be "range" or "number"
        };
...
fn noderef_reactivity_test() {
    create_scope_immediate(|cx| {
        ...
        let node_ref: &NodeRef = create_node_ref(cx);
...
fn dyn_fragment_reuse_nodes() {
    create_scope_immediate(|cx| {
        ...
        sycamore::render_to(&test_container(), {
