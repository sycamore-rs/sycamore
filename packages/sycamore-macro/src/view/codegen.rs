...
impl Codegen {
    pub fn view_root(&self, view_root: &ViewRoot) -> TokenStream {
        ...
        let quoted = match &view_root.0[..] {
            ...
            nodes => {
                ...
                quote! {
                    ::sycamore::view::View::new_fragment(::std::vec![
                        #(#nodes),*
                    ])
                }
            }
    ...
    /// TODO: If the root is an element, create a `Template`.
    pub fn view_node(&self, view_node: &ViewNode) -> TokenStream {
        let cx = &self.cx;
        match view_node {
            ViewNode::Element(element) => {
                let node = codegen_element(&self.elements_mod_path, &self.cx, element);
                quote! { ::sycamore::view::View::new_node(#node) }
            }
            ViewNode::Component(component) => {
                codegen_component(&self.elements_mod_path, &self.cx, component)
            }
            ViewNode::Text(Text { value }) => quote! {
                ::sycamore::view::View::new_node(
                    ::sycamore::generic_node::GenericNode::text_node(#cx, ::std::borrow::Cow::Borrowed(#value))
                )
            },
...
//                 } else {
//                     let text = quote! {
// ::std::borrow::Cow::Owned(::std::string::ToString::to_string(&#expr)) };                     if
// is_dynamic {                         self.flagged_nodes_quoted.extend(quote! {
//                             let __el = ::std::clone::Clone::clone(&__flagged[#flag_counter]);
//                             ::sycamore::reactive::create_effect(#cx, move || {
//                                 ::sycamore::generic_node::GenericNode::set_attribute(&__el,
// ::std::borrow::Cow::Borrowed(#name), #text);                             });
//                         });
//                     } else {
//                         self.flagged_nodes_quoted.extend(quote! {
//
// ::sycamore::generic_node::GenericNode::set_attribute(&__flagged[#flag_counter],
// ::std::borrow::Cow::Borrowed(#name), #text);                         });
//                     }
//                     (None, true)
//                 }
...
//                         if #expr {
//                             ::sycamore::generic_node::GenericNode::set_attribute(&__el,
// ::std::borrow::Cow::Borrowed(#name), ::std::borrow::Cow::Borrowed(""));                         }
// else {                             ::sycamore::generic_node::GenericNode::remove_attribute(&__el,
...
//                     (
//                         Some(
//                             quote! { (#name, { let _e: ::std::primitive::bool = #expr;
...
//                         let __el = ::std::clone::Clone::clone(&__flagged[#flag_counter]);
//                         ::sycamore::reactive::create_effect(#cx, move ||
//                             ::sycamore::generic_node::GenericNode::set_property(&__el, #prop,
// &#value)                         );
//                     });
//                 } else {
//                     self.flagged_nodes_quoted.extend(quote! {
...
//                                 ::sycamore::reactive::Signal::set(&#expr,
...
//             AttributeType::Ref => {
//                 self.flagged_nodes_quoted.extend(quote! {
//                     ::sycamore::noderef::NodeRef::set(&#expr,
// ::std::clone::Clone::clone(&__flagged[#flag_counter]));                 });
//                 (None, true)
//             }
...
//                         let __el = ::std::clone::Clone::clone(&__flagged[#flag_counter]);
//                         ::sycamore::utils::apply_attribute(#cx, __el,
// ::std::clone::Clone::clone(&name), value);                     }
//                 });
//                 (None, true)
//             }
//         }
...
//         self.dyn_values.push(codegen_component(
//             &self.elements_mod_path,
//             &self.cx,
//             component,
//         ));
//         quote! {
...
// ::sycamore::view::ToView::to_view(&(#value)))             },
//             false => quote! {
...
fn codegen_element(elements_mod_path: &syn::Path, cx: &Ident, element: &Element) -> TokenStream {
    let Element {
        tag,
        brace: _,
        attrs,
    ...
    let tag_quoted = match tag {
        ElementTag::Builtin(tag) => quote! { #elements_mod_path::elements::#tag(#cx) },
        ElementTag::Custom(_) => todo!(),
    };
