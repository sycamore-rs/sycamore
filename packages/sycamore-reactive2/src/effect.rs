...
thread_local! {
    ...
    ///
    /// The [`Listener`] contains an array of callbacks that, when called, will add the a `Signal` to
    /// the `handle` in the argument. The callbacks return another callback which will unsubscribe the
    /// `handle` from the `Signal`.
    pub(super) static CONTEXTS: RefCell<Vec<Weak<RefCell<Option<Listener>>>>> =
    ...
    fn internal(initial: Box<InitialFn>) -> Box<dyn Any> {
        ...
        let execute: Rc<RefCell<dyn FnMut()>> = Rc::new(RefCell::new({
            ...
            move || {
                CONTEXTS.with(|contexts| {
                    ...
                    // Upgrade listener now to make sure listener is valid for the whole duration of
                    // the effect.
                    let listener = listener.upgrade().unwrap();

                    // Push new reactive scope.
                    contexts.borrow_mut().push(Rc::downgrade(&listener));
                    ...
                    if let Some(initial) = initial.take() {
                        ...
                        listener.borrow_mut().as_mut().unwrap().clear_dependencies();
        ...
        *listener.borrow_mut() = Some(Listener {
        ...
        debug_assert_eq!(
            ...
            1,
            "Listener should be owned exclusively by ReactiveScope"
        );
...
pub fn dependency_count() -> Option<usize> {
    CONTEXTS.with(|contexts| {
        contexts.borrow().last().map(|last_context| {
            last_context
                .upgrade()
                .expect("Listener should be valid while inside reactive scope")
                .borrow()
                .as_ref()
                .unwrap()
                .dependencies
