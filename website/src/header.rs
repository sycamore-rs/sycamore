...
#[component]
fn DarkModeToggle<G: Html>(cx: Scope) -> View<G> {
    ...
    view! { cx,
        button(
            ...
            dangerously_set_inner_html=if *dark_mode.get() { LIGHT_BULB_SVG } else { CLOUD_MOON_SVG },
        )
    }
}
...
#[component]
pub fn HamburgerMenu<G: Html>(cx: Scope) -> View<G> {
    ...
    let is_open = cx.create_signal(false);
    let toggle = |_| is_open.set(!*is_open.get());
    let sidebar = cx.use_context::<RcSignal<Option<(Option<String>, SidebarData)>>>();
